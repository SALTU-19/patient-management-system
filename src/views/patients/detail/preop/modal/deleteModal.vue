...
export default {
    ...
    props: ["showDeletePreOpModal", "uid"],
    emits: ["close-modal"],
    computed: {

    },
    watch: {
        showDeletePreOpModal(val) {
            if (val) {
                this.currentStep = 1;
    ...
    methods: {
        ...
        async _deletePreOp() {
            ...
            let response = await this.deletePreOp({ uid: this.uid });
        ...
        async finish() {
            ...
            isDone = await this._deletePreOp();
    ...
    async mounted() {
        ...
        // this.filteredPreOps = this.hospitals;
        this.resetForm();
    },
...
<template>
    <teleport to="body">
        <Modal title="Delete Pre-Op" :showModal="showDeletePreOpModal" @closeModal="$emit('close-modal')" noPadding
            fixedHeight :md="true">
            <template #content>
