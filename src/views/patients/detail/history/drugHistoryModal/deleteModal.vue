...
export default {
    ...
    props: ["showDeleteDrugHistoryModal", "uid"],
    emits: ["close-modal"],
    computed: {

    },
    watch: {
        showDeleteDrugHistoryModal(val) {
            if (val) {
                this.currentStep = 1;
    ...
    methods: {
        ...
        async _deleteDrugHistory() {
            this.isSaving = true;

            let response = await this.deleteDrugHistory({ uid: this.uid });
        ...
        async finish() {
            ...
            isDone = await this._deleteDrugHistory();
    ...
    async mounted() {
        // await this._getAllDrugHistorys();
        // this.filteredDrugHistorys = this.hospitals;
        this.resetForm();
    },
...
<template>
    <teleport to="body">
        <Modal title="Delete Drug History" :showModal="showDeleteDrugHistoryModal" @closeModal="$emit('close-modal')"
