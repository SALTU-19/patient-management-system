...
export default {
    name: "DeleteSurgeryHistoryModal",
    data() {
        return {
            isSaving: false,
            isError: false,
    ...
    props: ["showDeleteSurgeryHistoryModal", "uid"],
    emits: ["close-modal"],
    computed: {

    },
    watch: {
        showDeleteSurgeryHistoryModal(val) {
            if (val) {
                this.currentStep = 1;
    ...
    methods: {
        ...mapActions(["deleteSurgeryHistory"]),
        ...
        async _deleteSurgeryHistory() {
            this.isSaving = true;

            let response = await this.deleteSurgeryHistory({ uid: this.uid });
        ...
        async finish() {
            ...
            isDone = await this._deleteSurgeryHistory();
    ...
    async mounted() {
        // await this._getAllSurgeryHistorys();
        // this.filteredSurgeryHistorys = this.hospitals;
        this.resetForm();
    },
...
<template>
    <teleport to="body">
        <Modal title="Delete Surgery History" :showModal="showDeleteSurgeryHistoryModal" @closeModal="$emit('close-modal')"
