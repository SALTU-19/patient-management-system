...
export default {
    ...
    props: ["showDeletePostOpModal", "uid"],
    emits: ["close-modal"],
    computed: {

    },
    watch: {
        showDeletePostOpModal(val) {
            if (val) {
                this.currentStep = 1;
    ...
    methods: {
        ...
        async _deletePostOp() {
            ...
            let response = await this.deletePostOp({ uid: this.uid });
        ...
        async finish() {
            ...
            isDone = await this._deletePostOp();
    ...
    async mounted() {
        // await this._getAllPostOps();
        // this.filteredPostOps = this.hospitals;
        this.resetForm();
    },
...
<template>
    <teleport to="body">
        <Modal title="Delete Post-Op" :showModal="showDeletePostOpModal" @closeModal="$emit('close-modal')" noPadding
            fixedHeight :md="true">
            <template #content>
