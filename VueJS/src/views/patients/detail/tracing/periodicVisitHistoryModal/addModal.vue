...
export default {
    name: "AddPeriodicVisitHistoryModal",
    data() {
        return {
            vki: "",
            ewl: "",
    ...
    props: ["showAddPeriodicVisitHistoryModal", "patientUID"],
    emits: ["close-modal"],
    watch: {
        showAddPeriodicVisitHistoryModal(val) {
            if (val) {
                this.currentStep = 1;
    ...
    methods: {
        ...mapActions(["createPeriodicVisitHistory"]),
        ...mapMutations(["setCreatePeriodicVisitHistoryCreds"]),
        ...
        controlFields() {
            ...
            if (
                ...
                this.showAddPeriodicVisitHistoryModal
            ) {
        ...
        async _createPeriodicVisitHistory() {
            this.isSaving = true;




            this.setCreatePeriodicVisitHistoryCreds({
            ...
            let response = await this.createPeriodicVisitHistory();
        ...
        async finish() {
            ...
            isDone = await this._createPeriodicVisitHistory();
...
<template>
    <teleport to="body">
        <Modal title="Add Periodic Visit History" :showModal="showAddPeriodicVisitHistoryModal"
