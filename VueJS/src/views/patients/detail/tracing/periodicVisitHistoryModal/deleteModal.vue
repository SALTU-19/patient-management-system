...
export default {
    name: "DeletePeriodicVisitHistoryModal",
    data() {
        return {
            isSaving: false,
            isError: false,
    ...
    props: ["showDeletePeriodicVisitHistoryModal", "uid"],
    emits: ["close-modal"],
    computed: {

    },
    watch: {
        showDeletePeriodicVisitHistoryModal(val) {
            if (val) {
                this.currentStep = 1;
    ...
    methods: {
        ...mapActions(["deletePeriodicVisitHistory"]),
        ...
        async _deletePeriodicVisitHistory() {
            this.isSaving = true;

            let response = await this.deletePeriodicVisitHistory({ uid: this.uid });
        ...
        async finish() {
            ...
            isDone = await this._deletePeriodicVisitHistory();
    ...
    async mounted() {
        // await this._getAllPeriodicVisitHistorys();
        // this.filteredPeriodicVisitHistorys = this.hospitals;
        this.resetForm();
    },
...
<template>
    <teleport to="body">
        <Modal title="Delete Periodic Visit History" :showModal="showDeletePeriodicVisitHistoryModal"
